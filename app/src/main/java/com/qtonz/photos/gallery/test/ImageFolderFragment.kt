...
class ImageFolderFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Load recent images if tabPosition is 0, otherwise load images from the folder
        if (tabPosition == 0) {
            loadRecentImages()
        } else {
            folderPath?.let { loadImagesFromFolder(it) }
    ...
    // Load recent images from Room database
    private fun loadRecentImages() {
        val database = RecentlyOpenedImageDatabase.getDatabase(requireContext())
        database.recentlyOpenedImageDao().getAllRecentlyOpenedImages().observe(viewLifecycleOwner) { images ->
            val imagePaths = images.map { it.imagePath }
            adapter.updateImages(imagePaths) // Update adapter with the list of recent images
        }
    }
    ...
    private fun loadImagesFromFolder(folderPath: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            cursor?.use {
                ...
                withContext(Dispatchers.Main) {
                    adapter.updateImages(newImageList) // Update adapter with the images from the folder
                }
            }
        }
    }
    ...
    companion object {
        ...
        // Factory method to create a new instance of ImageFolderFragment
        fun newInstance(folderPath: String, tabPosition: Int): ImageFolderFragment {
            return ImageFolderFragment().apply {
